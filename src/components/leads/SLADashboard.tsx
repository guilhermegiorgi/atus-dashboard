...
export function SLADashboard({ className }: { className?: string }) {
  ...
  const complianceRate =
    total > 0 ? ((data.dentro_do_sla / total) * 100).toFixed(1) : "0.0";
  const complianceColor =
    Number(complianceRate) >= 90
      ...
      : Number(complianceRate) >= 70
        ...
        : "text-dd-accent-red";
  const responseColor =
    data.tempo_medio_resposta_min <= 30
      ? "text-dd-accent-green"
      : data.tempo_medio_resposta_min <= 60
        ? "text-dd-accent-orange"
        : "text-dd-accent-red";
    ...
    <div className={`space-y-6 ${className ?? ""}`}>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-3">
        <Card className="glass border-border/50">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {data.dentro_do_sla} de {total} no prazo
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="glass border-border/50">
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${responseColor}`}>
              {data.tempo_medio_resposta_min}min
            </div>
        ...
        <Card className="glass border-border/50 col-span-2 lg:col-span-1">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {data.fora_do_sla} fora do prazo
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="glass border-border/50">
        ...
        <CardContent>
          <div className="grid gap-4 grid-cols-2">
            <div className="rounded-xl border border-dd-border-subtle bg-dd-surface-raised p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-semibold text-dd-accent-green">
                    {data.dentro_do_sla}
                  </p>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-dd-border-subtle bg-dd-surface-raised p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-semibold text-dd-accent-red">
                    {data.fora_do_sla}
                  </p>
                </div>
              </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {Object.entries(data.por_origem).map(([origem, vals]) => {
                ...
                const pct =
                  origTotal > 0
                    ? ((vals.dentro / origTotal) * 100).toFixed(0)
                    : "0";
                return (
                  <div
                    key={origem}
                    className="flex items-center justify-between rounded-lg border border-dd-border-subtle bg-dd-surface-raised px-4 py-2.5"
                  >
                    <span className="text-sm capitalize">{origem}</span>
                    <div className="flex items-center gap-4 text-xs text-right">
                      <span className="text-dd-accent-green">
                        {vals.dentro} ✓
                      </span>
                      <span className="text-dd-accent-red">{vals.fora} ✗</span>
                      <span className="font-semibold text-dd-on-surface w-10">
                        {pct}%
                      </span>
                    </div>
                  </div>

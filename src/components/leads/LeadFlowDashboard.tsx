...
const STATUS_COLOR: Record<string, string> = {
  ...
  EM_ATENDIMENTO:
    "bg-dd-accent-blue/20 text-dd-accent-blue border-dd-accent-blue/30",
  AGUARDANDO_RETORNO:
    "bg-dd-accent-purple/20 text-dd-accent-purple border-dd-accent-purple/30",
  CONVERTIDO:
    "bg-dd-accent-green/20 text-dd-accent-green border-dd-accent-green/30",
    ...
    <div className={`space-y-6 ${className ?? ""}`}>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="glass border-border/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-dd-accent-green">
              {conversionRate}%
            </div>
        ...
        <Card className="glass border-border/50 col-span-2 lg:col-span-1">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Últimos {data.conversoes_semana.length} dias
            </p>
          </CardContent>
        </Card>
      </div>
                ...
                {Object.entries(data.por_status).map(([status, count]) => (
                  ...
                  >
                    <Badge
                      ...
                      className={
                        STATUS_COLOR[status] ??
                        "bg-dd-surface-overlay/20 text-dd-on-muted"
                      }
                    >
                      {STATUS_LABEL[status] ?? status}
                    </Badge>
                    <div className="flex items-center gap-3">
                      <div className="h-1.5 w-24 rounded-full bg-dd-surface-overlay overflow-hidden">
                        <div
            ...
            <div className="flex items-end gap-2 h-20">
              {data.conversoes_semana.map((item) => {
                const maxVal = Math.max(
                  ...data.conversoes_semana.map((d) => d.total),
                  1,
                );
                ...
                return (
                  ...
                  >
                    <span className="text-xs font-medium text-dd-on-surface">
                      {item.total}
                    </span>
                    <div
